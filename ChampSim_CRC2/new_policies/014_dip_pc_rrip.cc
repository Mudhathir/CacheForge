...
// PC Counter Table parameters (per‐PC reuse predictor)
#define PCCTR_BITS   5
...
// DIP parameters (global policy selector)
#define PSEL_BITS    10
#define PSEL_MAX     ((1 << PSEL_BITS) - 1)
#define PSEL_INIT    (PSEL_MAX / 2)
#define BRRIP_PROB   32                  // 1/32 MRU insertion in BRRIP

// Per‐block metadata
static uint8_t  rrpv        [NUM_CORE][LLC_SETS][LLC_WAYS];
static bool     valid       [NUM_CORE][LLC_SETS][LLC_WAYS];
static bool     M_flag      [NUM_CORE][LLC_SETS][LLC_WAYS]; // hit‐since‐fill tracker
static uint16_t pc_index_arr[NUM_CORE][LLC_SETS][LLC_WAYS]; // index into PCCT
static uint8_t  meta_flag   [NUM_CORE][LLC_SETS][LLC_WAYS]; // 0=heavy,1=SRRIP,2=BRRIP,3=stream

// PC Counter Table and global PSEL
    ...
    for (int c = 0; c < NUM_CORE; c++) {
        for (int s = 0; s < LLC_SETS; s++) {
            for (int w = 0; w < LLC_WAYS; w++) {
                ...
                meta_flag[c][s][w]    = 3; // default streaming
            }
        }
    }
...
uint32_t GetVictimInSet(
    ...
    // 2) SRRIP‐style: find way with RRPV == MAX_RRPV
    while (true) {
        for (uint32_t w = 0; w < LLC_WAYS; w++) {
...
void UpdateReplacementState(
    ...
    total_accesses++;
    if (hit) {
        // On hit: promote to MRU, mark reuse
        hit_accesses++;
        M_flag[cpu][set][way] = true;
        rrpv[cpu][set][way]   = 0;
        // If this was a moderate‐reuse block, update PSEL
        uint8_t mf = meta_flag[cpu][set][way];
        if (mf == 1) {            // SRRIP
    ...
    // On miss
    miss_accesses++;
    // 1) Eviction: update PCCT based on evicted block reuse
    if (valid[cpu][set][way]) {
        uint16_t old_idx = pc_index_arr[cpu][set][way];
        if (M_flag[cpu][set][way]) {
            if (PCCT[old_idx] < PCCTR_MAX) PCCT[old_idx]++;
        } else {
            if (PCCT[old_idx] > 0)         PCCT[old_idx]--;
        }
    }
    // 2) Install new block
    ...
    // 3) Three‐class insertion with DIP for moderate blocks
    uint8_t ctr = PCCT[idx];
    if (ctr > HIGH_THLD) {
        // Heavy‐reuse → MRU
        rrpv[cpu][set][way]    = 0;
        meta_flag[cpu][set][way] = 0;
    }
    else if (ctr <= LOW_THLD) {
        // Streaming/no reuse → near eviction
        rrpv[cpu][set][way]    = MAX_RRPV;
        meta_flag[cpu][set][way] = 3;
    }
    else {
        // Moderate reuse → use global DIP to choose SRRIP vs BRRIP
        if (PSEL >= PSEL_INIT) {
            // SRRIP insertion point
            rrpv[cpu][set][way]    = MAX_RRPV - 1;
            meta_flag[cpu][set][way] = 1;
        } else {
            // BRRIP: mostly near eviction, 1/BRRIP_PROB MRU
            bool do_mru = (((PC >> 2) & (BRRIP_PROB - 1)) == 0);
            rrpv[cpu][set][way]      = do_mru ? 0 : MAX_RRPV;
            meta_flag[cpu][set][way] = 2;
        }
    }
}
...
void PrintStats() {
    double hr = total_accesses
                ...
                : 0.0;
    std::cout << "==== DIP-PC-RRIP Final Stats ====\n";
