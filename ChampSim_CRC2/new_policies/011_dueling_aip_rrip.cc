...
void UpdateReplacementState(
    ...
    if (hit) {
        ...
        bool is_aip_leader   = ((set & (DUEL_SETS - 1)) == AIP_LEADER_PATTERN);
        ...
        // Choose insertion policy: AIP vs SRRIP
        bool use_aip;
        if (is_aip_leader) {
            use_aip = true;
        } else if (is_srrip_leader) {
            ...
            use_aip = (PSEL > (PSEL_MAX >> 1));
        }

        if (use_aip) {
            // Graded insertion: map SHCT[new_sig] into [0..RRPV_MAX]
            uint8_t ctr = SHCT[new_sig];
            uint8_t new_rrpv = (uint8_t)(RRPV_MAX - (ctr * RRPV_MAX / SHCT_CTR_MAX));
...
void PrintStats() {
    ...
    std::cout << "==== Dueling AIP-RRIP Stats ====\n";
